...
% by Oceane Guillot
% V3 --> EN CONDITION INITIALE ON NE GARDE QU'UN SEUL CYCLE, CHOISI AU HASARD PARMI LES CYCLES
% SUPÉRIEURE A UNE TAILLE MINIMALE. CE CYCLE EST LA CLOTURE INITIALE, ET
% LES VARIATIONS NE SE FONT QU'A PARTIR DE CETTE CLOTURE INITIALE (variation_2.m)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clearvars; clc
...
for a = 1:z
    if viab==1
        ...
        name = strcat('pdf_clo',num2str(a),'.m');
        %     save([strcat('pdf_clo',num2str(a)),'.m'],'pdf_clo')
        old_path_to_file = "pdf_clo.m";
        new_filename = name;
...
%         Evo_jetons_b = Evo_jetons(1:2:end,:) ; % pour ne pas voir quand les jetons vont dans les transitions
        Time = [Time, size(evo_jetons,1)]; % pour savoir le T de chaque phase de variation
%         Time_b = [Time, size(evo_jetons,1)/2]; % pour savoir le T de chaque phase de variation
    end
end %for
...
plot(log10(Evo_jetons),'LineWidth',3);
% plot(log10(Evo_jetons_b),'LineWidth',3);
...
%     time_tot = time_tot + Time_b(i);
%     h = plot([time_tot,time_tot],[0,max(max(Evo_jetons))],'--k','LineWidth',0.5);
    h = plot([time_tot,time_tot],[0,max(max(log10(Evo_jetons)))],'--k','LineWidth',0.5);
end
